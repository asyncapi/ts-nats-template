  ...
  return new Promise(async (resolve, reject) => {
    if (nc) {
      try {
        ...
        let subscription = nc.subscribe(`smartylighting.streetlights.1.0.action.${streetlightId}.turn.off`,async (err, msg) => {
          if (err) {
            onRequest(err);
          } else {
            const unmodifiedChannel = `smartylighting.streetlights.1.0.action.{streetlightId}.turn.off`
