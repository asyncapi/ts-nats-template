...
export function reply(
  onRequest: (err?: NatsError, msg?: DimLightMessage, streetlightId?: string) =>Promise<DimLightResponseMessage>, 
  ...
  return new Promise(async (resolve, reject) => {
    if (nc) {
      try {
        ...
        let subscription = nc.subscribe(`smartylighting.streetlights.1.0.action.${streetlightId}.dim`,async (err, msg) => {
          if (err) {
            onRequest(err);
          } else {
            const unmodifiedChannel = `smartylighting.streetlights.1.0.action.{streetlightId}.dim`
            ...
            let requestData = DimLightMessage.toMessage(processedDataReceived);
