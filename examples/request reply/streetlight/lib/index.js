...
class NatsAsyncApiClient extends events.EventEmitter {
    ...
    replyToStreetlightStreetlightIdCommandTurnon(onRequest, onReplyError, streetlight_id, flush, options) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (nc) {
                try {
                    const sub = yield streetlightStreetlightIdCommandTurnonChannel.reply(onRequest, onReplyError, nc, streetlight_id, options);
                    if (flush) {
     ...
     * @param streetlight_id parameter to use in topic
     */
    requestStreetlightStreetlightIdEventTurnon(requestMessage, streetlight_id) {
        const nc = this.jsonClient;
        if (nc) {
            return streetlightStreetlightIdEventTurnonChannel.request(requestMessage, nc, streetlight_id);
        }
        else {
