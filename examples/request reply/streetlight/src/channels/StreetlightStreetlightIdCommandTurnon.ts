...
export function reply(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let subscription = await client.subscribe(`streetlight.${streetlight_id}.command.turnon`, async (err: any, msg) => {
        if (err) {
          onRequest(err);
        } else {
          ...
          let message = await onRequest(undefined, AnonymousSchema_1.unmarshal(receivedData), streetlightIdParam);
          if (msg.reply) {
